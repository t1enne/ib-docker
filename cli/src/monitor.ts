...
class WatchlistMonitor {
  ...
  private async loadWatchlists() {
    try {
      const [err, r] = await attemptAsync(() =>
        client.get("iserver/watchlists", { params: { SC: "USER_WATCHLIST" } }),
      );
      ...
      this.watchlists = r!.data.data.user_lists as Watchlist[];
  ...
  private async loadWatchlistInstruments(watchlistId: string) {
    try {
      const [err, r] = await attemptAsync(() =>
        client.get("iserver/watchlist", { params: { id: watchlistId } }),
      );
      ...
      this.currentInstruments = r!.data.instruments as Instrument[];
