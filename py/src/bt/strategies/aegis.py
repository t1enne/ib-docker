...
def plot(state: BacktestState, config: object) -> object:
    ...
    for symbol in strategy_config.symbols:
        ...
        if df is None or len(df) < params.mom_lookback_days:
            continue
        ...
        ram = pd.Series(
            [
                _vam(
                    closes.iloc[:i],
                    params.mom_lookback_days,
                    params.skip_days,
                    params.vol_window_days,
                )
                for i in range(params.mom_lookback_days, len(closes) + 1)
            ],
            index=closes.index[params.mom_lookback_days :],
        )
        overlays.setdefault(symbol, {})["vam"] = ram
