"""GMV (global minimum-variance) correlation-driven portfolio strategy.

Replicates the removed ``bt pf --weight-method gmv`` scheme as a `bt run`-able
strategy. At each rebalance boundary it holds the min-variance weights derived
from the trailing-return covariance of a whole universe (the allocation that
minimises portfolio variance given the correlation structure). Correlated
assets are jointly downweighted; diversifiers are upweighted. Long-only;
undeployed capital is held as cash. Single-interval (``1d``) universe backtests.
...
from src.bt.strategies.portfolio_weights import min_variance_weights
from src.bt.strategies.types import StrategyParams

STRATEGY_TYPE = "pf_gmv"
